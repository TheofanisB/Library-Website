<center>
	...
	<?php 
		...
		if(mysqli_num_rows($doresult2)==0){
			...
			$toDo2 =	"insert into announcement(author_id,type, title, year, month, note, url, announcement_key) values ((SELECT author_id from author WHERE surname='$surname'),'other', '$atitle', '$year', '$month', '$note', '$in_url', '$key')";
		...
		$result4 =	"select ISBN from inbook where (ISBN='$isbn')";
		...
		if(mysqli_num_rows($doresult3)==0){
			$toDo3 = "insert into inbook(announcement_id,publisher,chapter,pages,volume,number,series,type,address,edition,isbn,issn) values ((SELECT announcement_id from announcement WHERE title='$atitle'),'$title', '$chapter','$pages','$volume','$number', '$series', '$type', '$address', '$edition', '$isbn','$issn')";
			$result5 = mysqli_query($link, $toDo3) or die(header("HOUSTON WE HAVE A PROBLEM DEN MPOROUSAME NA VROUME TOUS USERS ENO EXOUME SINDETHEI ME TIN VASI todo2 "));
			echo "<p>Inbook was succesfully submitted!</p>";
			
		}else{
			$message = "NOT INSERTED!";
			echo "<script type='text/javascript'>alert('$message');</script>";
			echo "<p>Inbook wasn't succesfully submitted!</p>";
		}
	?>
	<p>efiga</p>
