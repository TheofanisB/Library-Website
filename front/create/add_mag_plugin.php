	...
	<center>
		...
		<?php 
			...
			if(mysqli_num_rows($doresult2)==0){
				$toDo2 =	"insert into announcement(author_id, type, title, year, month, note, url, announcement_key) values ((SELECT author_id from author WHERE surname='$surname'),'magazine', '$atitle', '$year', '$month', '$note', '$m_url', '$key')";
			...
			$toDo3 = "insert into article(announcement_id, journal, volume, number, pages, doi)values ((SELECT announcement_id from announcement WHERE title='$atitle'), '$journal', '$volume','$number','$pages', '$doi')";
			
			$result4 = mysqli_query($link, $toDo3) or die(header("HOUSTON WE HAVE A PROBLEM DEN MPOROUSAME NA VROUME TOUS USERS ENO EXOUME SINDETHEI ME TIN VASI todo3 "));
		?>
		<body>
			<p>Magazine was succesfully submitted!</p>
